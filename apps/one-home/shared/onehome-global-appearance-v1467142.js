(function(){
'use strict';
if(window.__ONEHOME_GLOBAL_APPEARANCE_V1467142__)return;
window.__ONEHOME_GLOBAL_APPEARANCE_V1467142__=true;
...
function scanRoot(root){if(!root)return;if(root.nodeType===1)neutralize(root);if(root.querySelectorAll)root.querySelectorAll('*').forEach(neutralize);}
function flushScan(){scanQueued=false;if(readExplicit()!==false&&document.documentElement.getAttribute('data-onehome-neon-enabled')!=='0'){pendingRoots.clear();return;}var roots=Array.from(pendingRoots);pendingRoots.clear();if(!roots.length)roots=[document.body||document.documentElement];if(observer)observer.disconnect();roots.forEach(scanRoot);if(observer)observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['style','class']});}
...
function ensureObserver(){if(observer||!window.MutationObserver)return;observer=new MutationObserver(function(records){if(document.documentElement.getAttribute('data-onehome-neon-enabled')!=='0')return;records.forEach(function(record){if(record.type==='attributes')queueScan(record.target);else Array.prototype.forEach.call(record.addedNodes||[],function(node){if(node&&node.nodeType===1)queueScan(node);});});});observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['style','class']});}
...
function refreshFromPreference(){var explicit=readExplicit();return apply(explicit===null?serverDefault:explicit,readColor(),false,false);}
async function loadServerDefault(){try{var url=SUPABASE_URL+'/rest/v1/'+TABLE+'?select=value&element_key=eq.'+encodeURIComponent(APPEARANCE_KEY)+'&is_active=eq.true&limit=1';var response=await fetch(url,{headers:{apikey:SUPABASE_KEY,Authorization:'Bearer '+SUPABASE_KEY},cache:'no-store'});if(response.ok){var rows=await response.json();var value=rows&&rows[0]&&rows[0].value;if(value&&typeof value==='object'){serverDefault=value.default_neon_on===true;serverColor=COLORS[String(value.default_neon_color||'').toLowerCase()]?String(value.default_neon_color).toLowerCase():'pink';}}}catch(_e){}refreshFromPreference();}
...
function oneHomeFixStandaloneHubLinks(){
  if(document.getElementById('onehomeEntryChoicePage'))return;
  document.querySelectorAll('a,button').forEach(function(control){
    var img=control.querySelector&&control.querySelector('img[alt="One Home"],img[src*="one-home-logo"],img[src*="one-home-return-logo"]');
    var label=String(control.getAttribute&&control.getAttribute('aria-label')||control.getAttribute&&control.getAttribute('title')||'').toLowerCase();
    if(!img&&!/one home choices|return to one home/.test(label))return;
    if(control.tagName==='A')control.setAttribute('href','/one-home/');
    if(control.dataset)control.dataset.onehomeHubDestination='choices';
  });
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',oneHomeFixStandaloneHubLinks,{once:true});else oneHomeFixStandaloneHubLinks();
window.addEventListener('pageshow',oneHomeFixStandaloneHubLinks);
})();
