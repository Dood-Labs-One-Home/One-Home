(function(){
'use strict';
if(window.__ONEHOME_DOODU_V1467262__)return;
window.__ONEHOME_DOODU_V1467262__=true;
var LESSONS={
 A:{"word":"Apple","prompt":"Make the A sign, then color the apple."},
 B:{"word":"Bowling Buddy","prompt":"Make the B sign, then color the bowling buddy."},
 C:{"word":"Cat","prompt":"Make the C sign, then color the cat."},
 D:{"word":"Dog","prompt":"Make the D sign, then color the dog."},
 E:{"word":"Elephant","prompt":"Make the E sign, then color the elephant."},
 F:{"word":"Fish","prompt":"Make the F sign, then color the fish."},
 G:{"word":"Garden","prompt":"Make the G sign, then color the garden."},
 H:{"word":"Horse","prompt":"Make the H sign, then color the horse."},
 I:{"word":"Ice Cream","prompt":"Make the I sign, then color the ice cream."},
 J:{"word":"Jellyfish","prompt":"Trace J in the air, then color the jellyfish."},
 K:{"word":"Kite","prompt":"Make the K sign, then color the kite."},
 L:{"word":"Lion","prompt":"Make the L sign, then color the lion."},
 M:{"word":"Monkey","prompt":"Make the M sign, then color the monkey."},
 N:{"word":"Nest","prompt":"Make the N sign, then color the nest."},
 O:{"word":"Octopus","prompt":"Make the O sign, then color the octopus."},
 P:{"word":"Pig","prompt":"Make the P sign, then color the pig."},
 Q:{"word":"Quail","prompt":"Make the Q sign, then color the quail."},
 R:{"word":"Rabbit","prompt":"Make the R sign, then color the rabbit."},
 S:{"word":"Snake","prompt":"Make the S sign, then color the snake."},
 T:{"word":"Tiger","prompt":"Make the T sign, then color the tiger."},
 U:{"word":"Unicorn","prompt":"Make the U sign, then color the unicorn."},
 V:{"word":"Van","prompt":"Make the V sign, then color the van."},
 W:{"word":"Whale","prompt":"Make the W sign, then color the whale."},
 X:{"word":"Xylophone","prompt":"Make the X sign, then color the xylophone."},
 Y:{"word":"Yo-Yo","prompt":"Make the Y sign, then color the yo-yo."},
 Z:{"word":"Zebra","prompt":"Trace Z in the air, then color the zebra."}
};
var ORDER=Object.keys(LESSONS),ASSETS='/assets/doodu/v181/',CARD_ASSETS='/assets/doodu/asl-262/',current='A';
...
function renderLanding(){
 var p=ensurePage('doodUPage');if(p.dataset.dooduVersion==='1467262')return;
...
function renderCourse(){var p=ensurePage('doodUCoursePage');if(p.dataset.dooduVersion==='1467262')return;p.innerHTML=header()+'<div class="doodu-children-stage"><div class="doodu-course-shell">'+wordmark()+'<div class="doodu-crumb">Children’s Learning → Sign &amp; Color Adventure</div><div class="doodu-pill">ASL Alphabet • Volume 1</div><h1>Choose a letter</h1><p>Practice each letter and color its card.</p>'+alphabet('')+'</div></div>';p.dataset.dooduVersion='1467262';bind(p)}
function renderLetter(l){var p=ensurePage('doodULetterPage'),d=LESSONS[l];if(p.dataset.dooduLetter===l&&p.dataset.dooduVersion==='1467262')return;
p.innerHTML=header()+'<div class="doodu-children-stage"><div class="doodu-letter-layout"><section class="doodu-left">'+wordmark()+'<button class="doodu-crumb" type="button" id="dooduCourseBack">⌂ &nbsp; Children’s Learning → Sign &amp; Color Adventure</button><div class="doodu-pill">ASL Alphabet • Volume 1</div><h1 class="doodu-lesson-title"><span>'+l+'</span> is for '+esc(d.word)+'</h1><p class="doodu-prompt">'+esc(d.prompt)+'</p><div class="doodu-activity-row">'+mascot()+'<div class="doodu-actions"><button class="doodu-action green" data-action="full">↓ &nbsp; Download 3×3 Card</button><button class="doodu-action pink" data-action="color">● &nbsp; Color Online</button></div></div>'+alphabet(l)+'<p class="doodu-status" role="status"></p></section><section class="doodu-preview" aria-label="ASL alphabet card preview"><img class="doodu-page-art" src="'+artwork(l)+'" width="900" height="900" fetchpriority="high" decoding="async" alt="'+l+' is for '+esc(d.word)+' — ASL alphabet coloring card with the Dood U duck"></section></div></div>';
p.dataset.dooduVersion='1467262';p.dataset.dooduLetter=l;bind(p);q('dooduCourseBack').onclick=openCourse;
p.querySelectorAll('[data-action]').forEach(function(b){b.onclick=function(){var f={full:downloadFullPage,color:colorOnline}[b.dataset.action];f(l)}});
...
async function downloadFullPage(l){
 l=valid(l);status('Preparing your card…');
 try{
  var response=await fetch(artwork(l),{cache:'no-store'});
  if(!response.ok)throw new Error('The picture could not load. Please try again.');
  var b=await response.blob();
  if(b.type!=='image/png'&&b.type!=='application/octet-stream')throw new Error('The image was not available. Please try again.');
  saveBlob(b,'Dood-U-ASL-'+l+'-3x3.png');status('Your 3×3 card is ready.');
...
modal.querySelector('[data-save]').onclick=async function(){
 var button=this,message=modal.querySelector('.doodu-save-status');button.disabled=true;message.textContent='Saving your colored page…';
 try{
  var out=document.createElement('canvas');out.width=c.width;out.height=c.height;var o=out.getContext('2d');
  o.drawImage(c,0,0);o.globalCompositeOperation='multiply';o.drawImage(base,0,0,out.width,out.height);
  var blob=await new Promise(function(resolve){out.toBlob(resolve,'image/png')});
  saveBlob(blob,'Dood-U-ASL-'+l+'-Colored-Card.png');message.textContent='Your colored card is ready. Check your downloads.';
 }catch(error){message.textContent='Your coloring is still here. The download could not be saved. Please try again.';}
 finally{button.disabled=false;}
};
}
function init(){ensurePage('doodUCoursePage');ensurePage('doodULetterPage');renderLanding()}
window.OneHomeDoodU={home:home,openCourse:openCourse,openLetter:openLetter,downloadFullPage:downloadFullPage,fullPage:downloadFullPage,colorOnline:colorOnline,lessons:LESSONS};
