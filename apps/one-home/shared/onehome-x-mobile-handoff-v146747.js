/* One Home v14.67.47 — X mobile EVM handoff + supported Stay in X path.
...
(function(){
  'use strict';
  if(window.OneHomeXMobileHandoff&&window.OneHomeXMobileHandoff.version==='14.67.47')return;

  const VERSION='14.67.47';
  const DISMISS_KEY='onehome_x_mobile_handoff_dismiss_v146747:';
  const STAY_KEY='onehome_x_stay_browser_v146747';
  ...
  function wasDismissed(){
    try{if(sessionStorage.getItem(dismissKey())==='1')return true;}catch(_e){}
    try{
      const raw=sessionStorage.getItem(STAY_KEY)||localStorage.getItem(STAY_KEY)||'';
      const saved=JSON.parse(raw||'null');
      if(saved&&saved.source==='x'&&Date.now()-Number(saved.saved_at||0)<2*60*60*1000){
        const current=campaignId();
        if(!current||!saved.campaign_id||String(saved.campaign_id).toLowerCase()===String(current).toLowerCase())return true;
      }
    }catch(_e){}
    return false;
  }
  function saveStayIntent(){
    const target=currentTarget();
    const payload={
      source:'x',
      campaign_id:campaignId(),
      exact_url:target?target.pathname+target.search+target.hash:'',
      saved_at:Date.now()
    };
    const encoded=JSON.stringify(payload);
    try{sessionStorage.setItem(STAY_KEY,encoded);}catch(_e){}
    try{localStorage.setItem(STAY_KEY,encoded);}catch(_e){}
  ...
  function render(){
    ...
    wrap.innerHTML=`<section class="ohx-card">
      ...
      <h2 id="oneHomeXMobileHandoffTitle">Choose how to continue this mint</h2>
      <p>You opened an EVM mint from X's mobile browser. You can continue in MetaMask now, or stay in X and sign in to One Home first.</p>
      <p class="ohx-note">If you stay in X, One Home keeps this exact mint and <strong>X source attribution</strong>, then opens MetaMask only when wallet approval is needed.</p>
    ...
    document.getElementById('oneHomeXStayHere')?.addEventListener('click',stayInX);
  }
  function init(){
  ...
  window.OneHomeXMobileHandoff={version:VERSION,render:render,metamaskUrl:metamaskUrl,dismiss:dismiss,stayInX:stayInX};
