...
(()=>{
'use strict';
function create({invoke,square=()=>window.Square,requireSandbox=true}={}){
  if(typeof invoke!=='function')throw new Error('One Home payment gateway is not configured.');
  const state=orderId=>invoke('onehome-square-card-payment',{action:'checkout_state',order_id:orderId});
  return Object.freeze({
    async config(){
      const cfg=await invoke('onehome-square-card-payment',{action:'config'});
    ...
    async mountCard(cfg,selector){
      if(!window.isSecureContext||location.protocol!=='https:')throw new Error('This card form requires trusted HTTPS. Restart the OH-249 secure launcher.');
    ...
    async charge({card,order,email,onSubmitting=()=>{}}){
      ...
      onSubmitting();
      return invoke('onehome-square-card-payment',{
