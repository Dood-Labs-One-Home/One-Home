/* One Home v14.67.44 — Universal mint source attribution + X sharing.
   Passport = identity. Wallet = signature. One Home = router.
   This module never posts to X for a user; it only opens X's official Web Intent composer. */
(function(){
  'use strict';
  if(window.OneHomeMintSource&&window.OneHomeMintSource.version==='14.67.60')return;

  const VERSION='14.67.60';
  ...
  function campaignIdFromLocation(){
    try{
      const path=text(location.pathname).match(/^\/mint\/([0-9a-f-]{36})(?:\/card-146760)?\/?$/i);
      if(path&&validCampaignId(path[1]))return path[1];
  ...
  function shareMintToX(options){
    ...
    let target=text(opts.url);
    if(target){
      try{
        const m=new URL(target,location.origin);
        if(m.origin===location.origin){m.searchParams.set('source','x');target=m.toString();}
      }catch(_error){}
    }
  ...
  function currentMintShareUrl(campaignId){
    ...
    try{
      ...
      ['campaign','preview','embedded','xaman_mint_payload','xaman_wallet_payload','return','oh_return','v'].forEach(function(key){u.searchParams.delete(key);});
      u.pathname='/mint/'+id;
      u.searchParams.set('source','x');
      u.searchParams.delete('card');
      u.searchParams.set('xcard','146760');
      return u.toString();
    }catch(_error){return location.origin+'/mint/'+id+'?source=x&xcard=146760';}
  }
