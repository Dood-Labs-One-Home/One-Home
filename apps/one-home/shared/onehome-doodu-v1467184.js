(function(){
'use strict';
if(window.__ONEHOME_DOODU_V1467184__)return;
window.__ONEHOME_DOODU_V1467184__=true;
var LESSONS={
...
function renderLanding(){
 var p=ensurePage('doodUPage');if(p.dataset.dooduVersion==='1467184')return;
 ...
 '</div></div>';
 p.dataset.dooduVersion='1467184';
}
...
function renderCourse(){var p=ensurePage('doodUCoursePage');if(p.dataset.dooduVersion==='1467184')return;p.innerHTML=header()+'<div class="doodu-children-stage"><div class="doodu-course-shell">'+wordmark()+'<div class="doodu-crumb">Children’s Learning → Sign &amp; Color Adventure</div><div class="doodu-pill">ASL Alphabet • Volume 1</div><h1>Choose a letter</h1><p>Make the sign, color a picture, and learn together.</p>'+alphabet('')+'</div></div>';p.dataset.dooduVersion='1467184';bind(p)}
function renderLetter(l){var p=ensurePage('doodULetterPage'),d=LESSONS[l];if(p.dataset.dooduLetter===l&&p.dataset.dooduVersion==='1467184')return;
p.innerHTML=header()+'<div class="doodu-children-stage"><div class="doodu-letter-layout"><section class="doodu-left">'+wordmark()+'<button class="doodu-crumb" type="button" id="dooduCourseBack">⌂ &nbsp; Children’s Learning → Sign &amp; Color Adventure</button><div class="doodu-pill">ASL Alphabet • Volume 1</div><h1 class="doodu-lesson-title"><span>'+l+'</span> is for '+esc(d.word)+'</h1><p class="doodu-prompt">'+esc(d.prompt)+'</p><div class="doodu-activity-row">'+mascot()+'<div class="doodu-actions"><button class="doodu-action green" data-action="full">↓ &nbsp; Download Full Page</button><button class="doodu-action pink" data-action="color">● &nbsp; Color Online</button></div></div>'+alphabet(l)+'<p class="doodu-status" role="status"></p></section><section class="doodu-preview" aria-label="Coloring page preview"><img class="doodu-page-art" src="'+artwork(l)+'" width="1102" height="1427" fetchpriority="high" decoding="async" alt="'+l+' is for '+esc(d.word)+' — ASL coloring page with the Dood Labs duck"></section></div></div>';
p.dataset.dooduVersion='1467184';p.dataset.dooduLetter=l;bind(p);q('dooduCourseBack').onclick=openCourse;
...
async function downloadFullPage(l){
 l=valid(l);status('Preparing your full page…');
 try{
  var img=await loadArtwork(l),c=document.createElement('canvas');
  ...
  var b=await new Promise(function(resolve){c.toBlob(resolve,'image/png')});
  saveBlob(b,'Dood-U-ASL-'+l+'-Full-Page.png');status('Your full-page PNG is ready.');
...
modal.querySelector('[data-save]').onclick=async function(){
 ...
 try{
  ...
  saveBlob(blob,'Dood-U-ASL-'+l+'-Colored-Page.png');message.textContent='Your colored-page PNG is ready. Check your downloads.';
