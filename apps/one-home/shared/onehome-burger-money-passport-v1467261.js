/* One Home OH-261 — Burger Money inherits the active One Home Passport.
...
(function () {
  ...
  function status(state, value) {
    if (current !== state || !state.status) return;
  ...
  async function history(state) {
    ...
    try {
      ...
      rows.filter(function (row) { return row.game_key === GAME_KEY; }).forEach(function (row) {
        ...
        item.textContent = 'Burger Money · ' + String(row.primary_score == null ? '?' : row.primary_score) +
  ...
  async function linkExistingPassport(state) {
    ...
    if (!sb || !sb.auth || !sb.auth.getSession) {
      status(state, 'Sign in to One Home to play.');
      state.linking = false;
      return;
    }
    try {
      ...
      status(state, 'Passport connection unavailable: ' + String(error && error.message || error));
    } finally {
      state.linking = false;
    }
  }
  ...
  function open(row) {
    ...
    if (window.OneHomeCatchFryPilot && typeof window.OneHomeCatchFryPilot.close === 'function') window.OneHomeCatchFryPilot.close();
    var overlay = document.createElement('div');
    overlay.className = 'onehome-game-stage-screen onehome-burger-money-screen';
    overlay.setAttribute('role', 'dialog');
    overlay.setAttribute('aria-modal', 'true');
    overlay.setAttribute('aria-label', 'Burger Money');
    overlay.innerHTML = '<header class="onehome-game-stage-bar"><button type="button" data-oh-close>← Back to Arcade</button>' +
      '<span class="onehome-game-stage-title">Burger Money</span><button type="button" data-oh-history-toggle aria-expanded="false">History</button></header>' +
      '<p class="onehome-game-stage-status" data-oh-status role="status" aria-live="polite" hidden></p>' +
      ...
      '<strong>Passport play history</strong><button type="button" data-oh-refresh disabled>Refresh</button></div>' +
    ...
    state.frame.title = 'Burger Money';
  ...
  window.OneHomeBurgerMoneyPilot = { open: open, close: close };
})();
