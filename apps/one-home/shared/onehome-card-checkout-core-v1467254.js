// One Home shared Square card checkout client (production integration).
...
(()=>{
'use strict';
function create({invoke,square=()=>window.Square,requireSandbox=false,paymentFunction='onehome-square-live-payment'}={}){
  ...
  return Object.freeze({
    async config(){
      ...
      if(cfg?.environment!=='production'||!cfg?.application_id||String(cfg.application_id).startsWith('sandbox-')){
        throw new Error('Square production configuration is not verified. Checkout is disabled.');
      }
    ...
    async mountCard(cfg,selector){
      if(!window.isSecureContext||location.protocol!=='https:')throw new Error('This card form requires trusted HTTPS. Please use the official secure One Home website.');
...
window.OneHomeProductionCardCheckout=Object.freeze({create});
})();
