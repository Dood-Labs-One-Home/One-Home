/* One Home v14.67.63 — Universal mint source attribution + dedicated X share bridge.
...
(function(){
  'use strict';
  if(window.OneHomeMintSource&&window.OneHomeMintSource.version==='14.67.63')return;

  const VERSION='14.67.63';
  const CARD_VERSION='146763';
  ...
  function sharePageUrl(options){
    ...
    const page=new URL('/mint-share/'+encodeURIComponent(id),location.origin);
    try{
      const dest=new URL(destination,location.origin);
      const chain=text(dest.searchParams.get('chain')).toLowerCase();
      const network=text(dest.searchParams.get('network')).toLowerCase();
      const route=text(dest.searchParams.get('route')).toLowerCase();
      if(chain)page.searchParams.set('chain',chain);
      else if(network==='mainnet'||network==='testnet')page.searchParams.set('network',network);
      if(route==='rare-routes')page.searchParams.set('route','rare-routes');
    }catch(_error){}
