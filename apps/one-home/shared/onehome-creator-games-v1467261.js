(function(){
'use strict';
if(window.__ONEHOME_CREATOR_GAMES_V1467260__)return;window.__ONEHOME_CREATOR_GAMES_V1467260__=true;
...
function openInfo(row,kind){
  ...
  if(kind==='arcade'&&String(row.game_key||'').toLowerCase()==='burger_money'&&window.OneHomeBurgerMoneyPilot){window.OneHomeBurgerMoneyPilot.open(row);return;}
  if(kind==='arcade'&&String(row.game_key||'').toLowerCase()==='catch_and_fry'&&window.OneHomeCatchFryPilot){window.OneHomeCatchFryPilot.open(row);return;}
