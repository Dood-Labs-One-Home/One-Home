/* OH-255 — Existing Passport, 6 FULL future feature months, reserve before payment.
...
let sb=null,me=null,checkout=null,order=null,card=null,rows=[],months=[],busy=false,current=null;
const monthLabel=m=>new Date(m+'T12:00:00Z').toLocaleDateString('en-US',{month:'long',year:'numeric',timeZone:'UTC'});
...
function template(){const host=$('onehomeShrunkCollabPage')?.querySelector('.oh-shrunk-room');if(!host||$('oh254LiveFlow'))return;
  ...
  flow.innerHTML=`<section class="oh254-panel"><h2>SHRUNK Collaborations</h2><div id="oh254Identity" class="oh254-status" role="status">Checking your One Home Passport…</div>
    ...
    <label>Feature month (a complete calendar month, starting next month)<select id="oh255Month" name="feature_month" required><option value="">Choose a feature month</option></select></label>
    <p id="oh255MonthsMessage" class="oh254-note" role="status">Checking the next six feature months…</p>
    ...
    <p class="oh254-note">Review your details before submitting. Your order is saved under your existing One Home Passport. Your chosen month is held for 15 minutes when you open checkout. Payment must begin before that hold expires.</p>
...
async function fetchMine(){const out=await room('mine');rows=out.collaborations||[];
  $('oh254List').replaceChildren();for(const item of rows){const wrap=document.createElement('div');wrap.className='oh254-item';const detail=document.createElement('div');detail.innerHTML=`<strong>${esc(item.display_name)}</strong><small>${esc(monthLabel(item.feature_month))} · ${item.payment_status==='paid'?'Booked and paid':item.booking_state==='review'?'Payment review':'Payment not complete'}</small>`;
...
async function loadMonths(){
 const result=await room('months');months=result.months||[];
 const picker=$('oh255Month');if(!picker)return;
 const previous=picker.value;picker.replaceChildren(new Option('Choose a full feature month',''));
 for(const m of months){const text=`${monthLabel(m.feature_month)} — ${m.remaining} of ${m.capacity} spots available`;
  const opt=new Option(text,m.feature_month);opt.disabled=!m.sales_open||!m.booking_open||m.remaining<=0;picker.add(opt);}
 if(months.some(m=>m.feature_month===previous&&m.sales_open&&m.booking_open&&m.remaining>0))picker.value=previous;
 $('oh255MonthsMessage').textContent='Each collaboration runs from the 1st through the last day of its selected month. Only the next six full months can be booked; availability may change before checkout.';
}
async function refresh(){try{
  ...
  status('oh254Availability','Choose a full feature month. Secure card checkout is available.','good');$('oh254Application').hidden=false;
  const field=$('oh254Form').elements.contact_email;if(!field.value)field.value=me.email||'';
  await fetchMine();
}catch(e){status('oh254Availability',e.message||'New card checkout is paused.','warn');$('oh254Application').hidden=true;await fetchMine().catch(()=>null);}}
...
async function open(item){if(busy||!safeUuid(item?.id))return;busy=true;current=item;order=null;
  ...
  try{const prep=await room('prepare',{collaboration_id:item.id});const id=prep.generic_order?.order_id;if(!safeUuid(id))throw Error('One Home did not return an order.');
    ...
    $('oh254Price').textContent=`${money(order.amount_cents)} · ${monthLabel(item.feature_month)}`;
    if(prep.reservation_expires_at)status('oh254CheckoutStatus',`Feature month reserved for 15 minutes. Begin payment before ${new Date(prep.reservation_expires_at).toLocaleTimeString([], {hour:'numeric',minute:'2-digit'})}.`,'good');
    if(order.order_status==='paid'){
      await room('sync',{order_id:order.id});$('oh254CardFields').hidden=true;status('oh254CheckoutStatus',`Payment confirmed. Your ${monthLabel(item.feature_month)} collaboration is booked.`,'good');await fetchMine();return;
    }
    ...
    $('oh254Pay').textContent=`Pay ${money(order.amount_cents)} securely`;$('oh254Pay').disabled=false;status('oh254CheckoutStatus','Review the total and enter your card in the Square form before your reservation expires.','good');
...
async function pay(){if(busy||!me||!order||!card)return;busy=true;$('oh254Pay').disabled=true;let submitted=false;
  ...
  try{await checkout.charge({card,order,email:me.email,onSubmitting(){submitted=true;}});
    ...
    status('oh254CheckoutStatus',`Payment confirmed. Your ${monthLabel(current.feature_month)} collaboration is booked.`,'good');await fetchMine();await loadMonths();
