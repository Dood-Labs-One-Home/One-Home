// One Home v14.67.218 — canonical Open Mints runtime: equal-size cards + authoritative live price/availability.
(function(){
  'use strict';
  ...
  async function refreshOpenDetail(force){
    ...
    if(!mint||overlay.hidden||isTestMint(mint))return;
  ...
  function openDetails(index,sourceButton){
    ...
    else{const creatorName=String(mint.creator_display_name||mint.creator||'').trim();creatorLine.textContent=creatorName?'By '+creatorName:'';creatorLine.hidden=!creatorName;}
    document.getElementById('detailDescription').textContent=mint.description||'Open this mint to view the collection and choose your wallet.';
    const detailPrice=document.getElementById('detailPrice');
    const detailAvailable=document.getElementById('detailAvailable');
    if(test){
    ...
    document.getElementById('detailOpenMint').href=canonicalUrl(mint);
    document.getElementById('detailOpenMint').textContent=test?(testStatusLabel(mint)==='OPEN'?'Open Testnet Mint':'View Testnet Mint'):'Open Mint';
  ...
  async function load(){
    ...
    liveMints=publicDirectory.live||[];
    ...
    if(publicResult.status==='rejected'&&testResult.status==='rejected'){
      error.hidden=false;
      return;
    }
